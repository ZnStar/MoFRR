# from DiffRR.models.ddm_uncond import *
...
class MoE_model(object):
    ...
    def restore(self,test_loader):
        # 读入专家模型
        print('save four outputs of experts')
        ...
        with torch.no_grad():
            ...
            for i, (x, y, name) in enumerate(test_loader):
                ...
                output_white = self.model_white(x)['pred_x']
                image_folder_white = os.path.join(image_folder,'white')
                utils.logging.save_image(output_white, os.path.join(image_folder_white,f"{name[0]}"))    

                self.model_smooth.to(self.device)
                output_smooth = self.model_smooth(x)['pred_x']
                image_folder_smooth = os.path.join(image_folder,'smooth')
                utils.logging.save_image(output_smooth, os.path.join(image_folder_smooth,f"{name[0]}"))    

                self.model_face.to(self.device)
                output_face = self.model_face(x)['pred_x']
                image_folder_face = os.path.join(image_folder,'face')
                utils.logging.save_image(output_face, os.path.join(image_folder_face,f"{name[0]}"))

                self.model_eye.to(self.device)
                output_eye = self.model_eye(x)['pred_x']
                image_folder_eye = os.path.join(image_folder,'eye')
                utils.logging.save_image(output_eye, os.path.join(image_folder_eye,f"{name[0]}"))
