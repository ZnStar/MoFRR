...
class HF_Crossattention(nn.Module):
    """
    input: xt(b,c,h,w), x(b,3c,h,w), t?, y_emb(3,64,64)?
    output: y(9,3,64,64)
    还没加条件，只是三个高频分别和xt做cross attention
    """
    def __init__(self, in_channels, out_channels):
        super(HF_Crossattention, self).__init__()
        self.conv_x = Depth_conv(in_channels, out_channels) 
        self.conv_xt = Depth_conv(in_channels, out_channels) 
    ...
    def forward(self, x, xt):
        ...
        # print(x.shape,xt.shape) # high0调用，[12,3,128,128],[4,3,64,64] high1调用 [12,3,64,64],[4,3,64,64]
        assert bt*3==b and ct==c
        residual = x
        x = self.conv_x(x)
        xt = self.conv_xt(xt)
        x_HL, x_LH, x_HH = x[:b//3, ...], x[b//3:2*b//3, ...], x[2*b//3:, ...]
        if x_HL.shape != xt.shape: 
            # 两次低频，因此这里需要调整xt大小适应xHL
            xt = F.interpolate(xt, size=(h, w), mode='bilinear', align_corners=False) 
        # print(x_HL.shape,xt.shape) # [4,64,128,128]经过深度卷积的结果
        # x_HL = self.dilated_block_HL(self.cross_attention1(x_HL, xt))
        # x_LH = self.dilated_block_LH(self.cross_attention2(x_LH, xt))
        # x_HH = self.dilated_block_HH(self.cross_attention3(x_HH, xt))
        x_HL = self.dilated_block_HL(self.cross_attention1(xt, x_HL))
        x_LH = self.dilated_block_LH(self.cross_attention2(xt, x_LH))
        x_HH = self.dilated_block_HH(self.cross_attention3(xt, x_HH))
        output = self.conv_tail(torch.cat((x_HL, x_LH, x_HH), dim=0))
