...
from MoE import RetouchingMulti,MoE_model,RetouchingMulti_all

def parse_args_and_config():
    parser = argparse.ArgumentParser(description='Evaluate Wavelet-Based Diffusion Model')
    ################## 需要修改 ############################
    parser.add_argument("--config", type=str, help="Path to the config file")
    parser.add_argument("--test_name", default='default', type=str,
                        help="test dataset name")
    parser.add_argument('--resume', default='', type=str,
                        help='Path for the diffusion model checkpoint to load for evaluation')
    # ### 单独测试文件夹
    parser.add_argument("--image_folder", default='/newdata/DiffRR/MoE/results/test', type=str,
...
if __name__ == '__main__':
    ...
    DATASET = RetouchingMulti_all(config) #要注意这个不是测试集而是全部
    test_loader = DATASET.get_loaders(test=True,test_folder=args.test_folder)
